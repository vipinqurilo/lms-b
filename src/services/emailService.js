...
const sendBookingConfirmation = async (bookingData) => {
  ...
  if (
    ...
    throw new Error(
      `Missing required fields for booking confirmation email: ${missingFields.join(
        ", "
      )}`
    );
  }
  ...
  const defaultData = {
    logoUrl:
      logoUrl ||
      "https://res.cloudinary.com/daprkakyk/image/upload/v1741260445/luxe/uiiqdcle3kayym5qg1kp.png",
    courseImage:
      courseImage ||
      "http://res.cloudinary.com/daprkakyk/image/upload/v1742290473/luxe/vqyts6joemuxzrlqv4uj.png",
    bookingDate,
    startTime,
    endTime,
    courseName,
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      teacherName,
      nextStepOne:
        "Your booking has been confirmed. We look forward to seeing you at the scheduled time.",
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      teacherName,
      nextStepOne:
        "A new session has been confirmed. Please be available 5 minutes before the scheduled time.",
      nextStepTwo:
        "You will get a reminder 15 minutes before the session starts.",
...
const sendBookingCancellation = async (bookingData) => {
  ...
  if (
    ...
    throw new Error(
      `Missing required fields for booking cancellation email. Required fields: ${[
        !studentEmail && "studentEmail",
        !studentName && "studentName",
        !teacherName && "teacherName",
        !teacherEmail && "teacherEmail",
  ...
  const defaultData = {
    ...
    courseImage:
      courseImage ||
      "http://res.cloudinary.com/daprkakyk/image/upload/v1742302660/luxe/k1b9kalcufrxti3rcjvs.png",
    bookingDate,
    startTime,
    endTime,
    courseName,
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      studentName,
      teacherName,
      nextStepOne: `Your booking has been cancelled ${
        cancelledBy === "student" ? "by you" : "by the teacher"
      }.`,
      nextStepTwo:
        "100% refund for the cancelled session will be processed within 7-10 working days.",
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      studentName,
      teacherName,
      nextStepOne: `The booking has been cancelled ${
        cancelledBy === "teacher" ? "by you" : "by the student"
      }.`,
...
const sendRescheduleRequest = async (bookingData) => {
  ...
  const defaultData = {
    ...
    courseImage:
      courseImage ||
      "http://res.cloudinary.com/daprkakyk/image/upload/v1742302524/luxe/qwq6d4zaigxlgs6mumoz.png",
    bookingDate,
    startTime,
    endTime,
    courseName,
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      requestedBy,
      nextStepOne: `${
        requestedBy === "student" ? "You have" : `${teacherName} has`
      } requested to reschedule this session.`,
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      requestedBy,
      nextStepOne: `${
        requestedBy === "teacher" ? "You have" : `${studentName} has`
      } requested to reschedule this session.`,
      nextStepTwo: `Proposed new time: ${newBookingDate} from ${newStartTime} to ${newEndTime}. Reason: ${rescheduleReason}`,
  ...
  return {
    success: true,
    message: "Reschedule request emails sent successfully",
  };
};
...
const sendRescheduleConfirmation = async (bookingData) => {
  ...
  if (
    ...
    throw new Error(
      "Missing required fields for reschedule confirmation email"
    );
  }
...
const sendRescheduleRejection = async (bookingData) => {
  ...
  return {
    success: true,
    message: "Reschedule rejection emails sent successfully",
  };
};
...
const sendBookingScheduled = async (bookingData) => {
  ...
  if (missingFields.length > 0) {
    throw new Error(
      `Missing required fields for booking scheduled email: ${missingFields.join(
        ", "
      )}`
    );
  }
  ...
  const defaultData = {
    ...
    courseImage:
      courseImage ||
      "http://res.cloudinary.com/daprkakyk/image/upload/v1742281964/luxe/jdpsyhelqvy74p1qvxw4.png",
    bookingDate,
    startTime,
    endTime,
    courseName,
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      teacherName,
      nextStepOne:
        "Your booking request has been scheduled and is awaiting for teacher confirmation.",
      nextStepTwo:
        "You'll receive a confirmation email once the teacher accepts your booking request.",
      buttonText: "View Booking Details",
    },
  });
  ...
  await sendTemplatedEmail({
    ...
    templateData: {
      ...
      teacherName,
      nextStepOne:
        "Please confirm or decline this booking session by providing meeting details. ",
      nextStepTwo:
        "The student will be notified about the confirmation of session.",
  ...
  return {
    success: true,
    message: "Booking scheduled emails sent successfully",
  };
};
